...
def main() -> None:
    ...
    # Signal to workflow whether polling should continue
    still_waiting = fetch_openai_batches_to_check(limit=1)
    still_pending = fetch_pending_jobs(limit=1)
    needs_polling = bool(still_waiting) or bool(still_pending)
    signal = "yes" if needs_polling else "no"
    with open("needs_polling.txt", "w") as f:
        f.write(signal)
    print(f"Needs polling signal: {signal}", flush=True)
